            ...
            <table class="table datatable">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td><a href="{{route('temuan_audit_detail_sudah_ditanggapi', ['id' => $item->id])}}">@php
                      ...
                      @endphp</a></td>
                  <td><a href="{{route('temuan_audit_detail_tinjau_ulang', ['id' => $item->id])}}">@php
                      $tanggapanAudit = $item->kertasKerjaAudit->filter(function($tanggapan) {
                      return $tanggapan->tanggapanAudit->where('status','0')->count() > 0;
                      });
                      echo $tanggapanAudit->count();
                      @endphp</a></td>
                  <td><a href="{{route('temuan_audit_detail_tindak_lanjut', ['id' => $item->id])}}">@php
                      $tanggapanAudit = $item->kertasKerjaAudit->filter(function($tanggapan) {
